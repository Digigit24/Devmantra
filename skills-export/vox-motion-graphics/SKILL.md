...
- **Palette not known and this is a generic/topical explainer with no brand
  ...
  described in `references/vox-prompts.md`.
...
STYLE KEY prompt in `references/vox-prompts.md`. Poll to completion; that
...
and the scene vocabulary in `references/vox-prompts.md`. Rules that are easy
to forget:
...
- **Motion must read as brisk, decisive, and stable — never slow or
  ...
  MOTION guidance in `references/vox-prompts.md`.
