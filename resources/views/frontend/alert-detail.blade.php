        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                <div class="dm-article-meta tp_fade_anim" data-delay=".3">
                    <span class="dm-article-meta-tag">{{ ucfirst($alert->tag) }} Alert</span>
                    <span class="dm-article-meta-item">{{ $alert->published_at?->format('M d, Y') ?? $alert->created_at->format('M d, Y') }}</span>
                    @if($alert->read_time)
                    <span class="dm-article-meta-item">{{ $alert->read_time }}</span>
                    @endif
                </div>
                <h1 class="dm-article-hero-title tp-text-revel-anim" data-delay=".5">{{ $alert->title }}</h1>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-lg-8">
                ...
                <div class="dm-article-footer">
                    <div class="dm-article-tags">
                        <a href="{{ route('alert.index', ['tag' => $alert->tag]) }}">{{ ucfirst($alert->tag) }} Alert</a>
                    </div>
                    <div class="dm-article-share">
                        <span>Share</span>
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($alert->title) }}" target="_blank" rel="noopener" aria-label="X"><i class="fa-brands fa-x-twitter"></i></a>
            ...
            <div class="col-lg-4">
                <div class="dm-sidebar">
                    ...
                    @foreach($sidebarItems as $sidePost)
                    <div class="dm-sidebar-post">
                        ...
                        <div class="dm-sidebar-post-info">
                            <span class="dm-sidebar-post-category">{{ ucfirst($sidePost->tag) }} Alert</span>
                            <h5 class="dm-sidebar-post-title">
                                <a href="{{ route('alert.show', $sidePost->slug) }}">{{ $sidePost->title }}</a>
                            </h5>
            ...
            <div class="col-lg-4 col-md-6">
                <div class="dm-related-card tp_fade_anim" data-delay=".{{ 3 + ($loop->index * 2) }}">
                    <div class="dm-related-card-thumb">
                        <a href="{{ route('alert.show', $post->slug) }}">
                    ...
                    <span class="dm-related-card-category">{{ ucfirst($post->tag) }} Alert</span>
                    <h4 class="dm-related-card-title">
                        <a href="{{ route('alert.show', $post->slug) }}">{{ $post->title }}</a>
                    </h4>
                </div>
            </div>
            @endforeach
