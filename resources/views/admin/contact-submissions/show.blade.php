...
@section('actions')
<a href="{{ route('admin.contact-submissions.index') }}" class="dm-btn dm-btn-outline dm-btn-sm">
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="dm-table-wrap" style="padding:24px;">
            ...
            <table class="dm-table" style="margin:0;">
                ...
                <tr>
                    ...
                    <td>{{ $submission->created_at->format('M d, Y \a\t h:i A') }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="dm-table-wrap" style="padding:24px;margin-top:20px;">
            ...
            <div style="background:var(--dm-bg);border:1px solid var(--dm-border);border-radius:8px;padding:20px;line-height:1.7;color:var(--dm-text);white-space:pre-wrap;">{{ $submission->message }}</div>
        </div>
    </div>
    <div class="col-lg-4">
        <div class="dm-table-wrap" style="padding:24px;">
            ...
            <a href="mailto:{{ $submission->email }}?subject=Re: {{ urlencode($submission->subject) }}" class="dm-btn dm-btn-outline w-100" style="margin-bottom:12px;">
                <i class="fa-solid fa-reply"></i> Reply via Email
            </a>

            <form method="POST" action="{{ route('admin.contact-submissions.destroy', $submission) }}" onsubmit="return confirm('Delete this submission permanently?')">
                @csrf @method('DELETE')
