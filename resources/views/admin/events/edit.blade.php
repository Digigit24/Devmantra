                ...
                <div class="dm-form-group">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:12px;margin-bottom:16px;">
                        ...
                        <div style="position:relative;border-radius:8px;overflow:hidden;border:1px solid rgba(0,0,0,0.08);">
                            <img src="{{$img->image_path }}" style="width:100%;height:100px;object-fit:cover;" alt="">
            ...
            <div class="dm-table-wrap" style="padding:24px;">
                <div class="dm-form-group">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:4px;">
                        @foreach(\App\Models\Event::TAGS as $value => $label)
                        @php $checked = in_array($value, old('tags', $event->tags ?? ['events'])); @endphp
                        <label style="display:inline-flex;align-items:center;gap:5px;cursor:pointer;font-size:13px;">
                            <input type="checkbox" name="tags[]" value="{{ $value }}" {{ $checked ? 'checked' : '' }}>
                            {{ $label }}
                        </label>
                        @endforeach
                    </div>
                ...
                <div class="dm-form-group">
                    ...
                    @if($event->featured_image)
                        <div class="mb-2">
                            <img src="{{  $event->featured_image }}" style="width:100%;border-radius:8px;max-height:160px;object-fit:cover;" alt="">
                        </div>
                    @endif
